...
net.Receive("hl2dm_respawn_start", function()
    surface.PlaySound("hl2mode1.wav")
    zb.RemoveFade()
    hg.DynaMusic:Start("hl_coop")
    livesRemaining = net.ReadInt(8)
end)

net.Receive("hl2dm_respawn_lives", function()
    livesRemaining = net.ReadInt(8)
end)

net.Receive("hl2dm_respawn_respawning", function()
    respawnTime = net.ReadFloat() + 5
    livesRemaining = net.ReadInt(8)
    
    hook.Add("HUDPaint", "HL2DM_RespawnTimer", function()
        if respawnTime < CurTime() then 
            hook.Remove("HUDPaint", "HL2DM_RespawnTimer") 
            return 
        end
        ...
        local formatted = string.FormattedTime(timeLeft, "%02i:%02i")
        
        -- Respawn timer
        draw.SimpleText("Respawning in " .. formatted, "ZB_HomicideMedium", sw * 0.5, sh * 0.75, Color(255, 200, 100, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        
        -- Lives remaining
        local livesColor = livesRemaining > 1 and Color(100, 255, 100) or Color(255, 100, 100)
        draw.SimpleText("Lives remaining: " .. livesRemaining, "ZB_HomicideMedium", sw * 0.5, sh * 0.8, livesColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end)
end)

local teams = {
    [0] = {
        objective = "Kill all combines and survive. You have multiple lives!",
        name = "a Rebel",
    ...
    [1] = {
        objective = "Destroy all rebel forces. You have multiple lives!",
...
function MODE:HUDPaint()
    -- Lives indicator (always visible when alive)
    if lply:Alive() and zb.ROUND_START + 8.5 < CurTime() then
        local livesColor = livesRemaining > 1 and Color(100, 255, 100, 200) or Color(255, 100, 100, 200)
        draw.SimpleText("Lives: " .. livesRemaining, "ZB_HomicideSmall", sw * 0.5, sh * 0.02, livesColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
    end
    ...
    draw.SimpleText("ZBattle | Half-Life 2 Deathmatch (Respawn)", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    ...
    -- Show lives count during intro
    local livesColor = Color(255, 255, 100)
    livesColor.a = 255 * fade
    draw.SimpleText("Lives: " .. livesRemaining, "ZB_HomicideMedium", sw * 0.5, sh * 0.55, livesColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
hook.Add("radialOptions", "CMB_Airstrike_Respawn", function()
    local org = lply.organism
    
    if lply:GetNWString("PlayerRole") == "Elite" and not org.otrub then
        local tbl = {
            function()
                net.Start("RequestAirstrike_Respawn") 
                net.SendToServer()
            end,
            "Request Airstrike"
        }
...
net.Receive("hl2dm_respawn_roundend", function()
...
function MODE:RoundStart()
    ...
    -- Clean up any lingering HUD hooks
    hook.Remove("HUDPaint", "HL2DM_RespawnTimer")
    hook.Remove("HUDPaint", "HL2DM_Eliminated")
end
